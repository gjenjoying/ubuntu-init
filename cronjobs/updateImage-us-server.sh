...
tccli lighthouse DescribeBlueprints --cli-unfold-argument --region ${tencentRegionUS} --Filters.0.Name blueprint-type --Filters.0.Values PRIVATE > image.json
...
tccli lighthouse DeleteBlueprints --cli-unfold-argument --region ${tencentRegionUS} --BlueprintIds $OldestImageId

sleep 30s

CurrentTimestamp=`date '+%s'`
tccli lighthouse CreateBlueprint --cli-unfold-argument --region ${tencentRegionUS} --BlueprintName $CurrentTimestamp --InstanceId ${tencentInstanceUSid} > image.json
cat image.json

NewImageId=$(jq .BlueprintId image.json)

sleep 120s
tccli lighthouse DescribeBlueprints --cli-unfold-argument --region ${tencentRegionUS} --Filters.0.Name blueprint-type --Filters.0.Values PRIVATE --Filters.1.Name blueprint-state --Filters.1.Values NORMAL > image.json
